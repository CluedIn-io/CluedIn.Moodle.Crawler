...
{
    public class GetCrawlJobDataBehaviour : MoodleProviderTest
    {
        ...
        [Theory]
        [InlineAutoData]
        public async Task MoodleCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.Moodle.Core.MoodleCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
